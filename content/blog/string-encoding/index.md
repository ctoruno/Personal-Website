...
The brilliant idea was to leave the core 127 ASCII characters (or graphemes) as they were, so the UNICODE could read them without any issue. But everything above the 127th code point had a standardized code, expressed in hex notation with "U+" as a prefix. This way, people using the English alphabet (which was and still is the dominant alphabet in the tech world) didn't noticed the change. You can check the character encodings in the [unicode website](https://home.unicode.org/).

In the previous example, the word _s-u-n-d-a-y_ will still be encoded the same way as it was in the ASCII system. But if we take a different word such as the portuguese "açúcar" (sugar) then you will start noticing the differences. See the image below.
